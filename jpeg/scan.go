...
func (d *decoder) processSOS(n int) error {
	...
	var scan [MAX_COMPS_IN_SCAN]struct {
		compIndex uint8
	...
	if d.img1 == nil && d.img3 == nil && d.img4 == nil {
	...
	for my := 0; my < myy; my++ {
		for mx := 0; mx < mxx; mx++ {
			// save results of nComp and put them in image afterwards, depending on type
			for i := 0; i < nComp; i++ {
				compIndex := scan[i].compIndex
				...
				for j := 0; j < d.comp[compIndex].h*d.comp[compIndex].v; j++ {
					...
					case RGBA:
						dst, stride = d.img4.Pix[8*(my0*d.img4.Stride+(mx0*4)):], d.img4.Stride
					default:
						return UnsupportedError("Unexpected")
					}
					...
					for y := 0; y < 8; y++ {
						...
						for x := 0; x < 8; x++ {
							...
							case RGBA:
								if (compIndex < 3) {
									// CMY
									dst[yStride+(x*4)+int(compIndex)] = shift(c)
								} else {
									// adjust image with K
									k := shift(c)
									dst[yStride+(x*4)+0] = byte(float32(dst[yStride+(x*4)+0]) * (float32(k) / 255.0))
									dst[yStride+(x*4)+1] = byte(float32(dst[yStride+(x*4)+1]) * (float32(k) / 255.0))
									dst[yStride+(x*4)+2] = byte(float32(dst[yStride+(x*4)+2]) * (float32(k) / 255.0))
								} 
							default:
								dst[yStride+x] = uint8(shift(c)) 
							}
						}
					}
				} // for j
